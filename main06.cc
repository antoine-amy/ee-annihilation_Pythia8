...
int counts = 0;
  ...
  for (int iEvent = 0; iEvent < 10000; ++iEvent) {
    ...
    counts += pythia.event.size(); //nbr de particule dans letat final
  ...
  double luminosity = 1.0; // arbitrary units
  double cross_section = counts / (luminosity * pythia.info.sigmaGen());
  std::cout << "Cross section: " << cross_section << " barns" << std::endl;
