...
int main(){
  ...
  for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
    ...
    if (thr.analyze( pythia.event )) {
      if (iEvent < nEvent) thr.list();
      thrust.fill( thr.thrust() );
      //oblateness.fill( thr.oblateness() );
      //tAxis.fill( thr.eventAxis(1).pz() );
      if (abs(thr.eventAxis(1).pAbs()-1.)>1e-8 || abs(thr.eventAxis(2).pAbs()-1.)>1e-8 || abs(thr.eventAxis(3).pAbs()-1.)>1e-8 || 
      abs(thr.eventAxis(1)*thr.eventAxis(2))>1e-8 || abs(thr.eventAxis(1)*thr.eventAxis(3))>1e-8 || abs(thr.eventAxis(2)*thr.eventAxis(3))>1e-8){
